...
function Products({ orders, setOrders, order, setOrder, productCount, setProductCount }) {
    const products = [{
            name: "necklace",
        ...
        const productMap = products.map(product => <AllProducts key={product.name} product={product} productCount={productCount} setProductCount={setProductCount} orders={orders} setOrders={setOrders} order={order} setOrder={setOrder}/>)
